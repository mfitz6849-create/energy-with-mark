...
test('native V3 transport is CORS, idempotency and acknowledgement guarded', () => {
  assert.match(runtime, /https:\/\/control\.energywithmark\.com\.au\/api\/public\/website-intake\/v1/);
...
test('booking requests go directly to V3 and remain requested until confirmed', () => {
  assert.match(booking, /https:\/\/control\.energywithmark\.com\.au\/api\/public\/website-intake\/v1/);
